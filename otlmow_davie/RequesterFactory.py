...
class RequesterFactory:
    ...
    @classmethod
    def create_requester(cls, auth_type: AuthType, env: Environment, settings_path: Path = None, cookie: str = None,
                         use_services: bool = True) -> AbstractRequester:
        first_part_url = cls.first_part_url_dict.get(env)
        if not use_services:
            if first_part_url is not None:
                first_part_url = first_part_url.replace('https://services.', 'https://')
